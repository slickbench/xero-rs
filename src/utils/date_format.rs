...
pub fn parse_dotnet_date(date_str: &str) -> Result<Date, String> {
    ...
    let format = format_description!("[year]-[month]-[day]");
    Date::parse(date_str, &format).map_err(|e| format!("Failed to parse date '{date_str}': {e}"))
}
...
pub mod xero_date_format {
    use super::{
        Date, Deserialize, Deserializer, Serializer, format_description, parse_dotnet_date, serde,
    };
...
pub mod xero_datetime_format {
        ...
        D: Deserializer<'de>,
    {
        ...
        super::parse_dotnet_datetime(&datetime_str).map_err(serde::de::Error::custom)
    }
}
