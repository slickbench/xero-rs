...
use xero_rs::invoice::{Builder, Type, ListParameters, ContactIdentifier};
...
async fn get_invoice() -> Result<()> {
    ...
    if !invoices.is_empty() {
        ...
        info!("Got invoice #{}: {:?}", invoice.invoice_id, invoice.invoice_number);
    } else {
...
async fn create_update_invoice() -> Result<()> {
    ...
    match client.invoices().update(created_invoice.invoice_id, &updated_builder).await {
        Ok(invoice) => {
            info!("Updated invoice: {} - {:?}", invoice.invoice_id, invoice.reference);
        },
        Err(e) => {
...
async fn invoice_history() -> Result<()> {
    ...
    if !invoices.is_empty() {
        ...
        match client.invoices().create_history(invoice_id, "Testing history creation").await {
...
async fn invoice_attachments() -> Result<()> {
    ...
    if !invoices.is_empty() {
        ...
        let _attachment = match client.invoices().upload_attachment(
            invoice_id, 
            filename, 
            test_file_content
        ).await {
        ...
        match client.invoices().list_attachments(invoice_id).await {
            Ok(attachments) => {
                ...
                if !attachments.is_empty() {
                    ...
                    match client.invoices().get_attachment(invoice_id, attachment_id).await {
                    ...
                    match client.invoices().get_attachment_by_filename(invoice_id, filename).await {
                    ...
                    match client.invoices().update_attachment(
                        invoice_id,
                        filename,
                        updated_content
                    ).await {
