    ...
    if let Some(payroll_calendar_id) = employee.payroll_calendar_id {
        ...
        let pay_calendar = match pay_calendar::get(client, payroll_calendar_id).await {
            Ok(calendar) => {
                ...
                info!("Pay calendar period: {} to {}", calendar.period_start_date, calendar.period_end_date);
                calendar
            },
            Err(e) => {
        ...
        info!("Using timesheet period from pay calendar: {} to {}", start_date, end_date);
        ...
        info!("Using earnings rate: ID={}, Name={}", 
                earnings_rate.earnings_rate_id, 
                earnings_rate.name);

        // Parse dates to calculate number of units
        let start = match NaiveDate::parse_from_str(&start_date, "%Y-%m-%d") {
            Ok(date) => date,
            Err(e) => return Err(miette::miette!("Failed to parse start date {}: {}", start_date, e)),
        };
        
        let end = match NaiveDate::parse_from_str(&end_date, "%Y-%m-%d") {
            Ok(date) => date,
            Err(e) => return Err(miette::miette!("Failed to parse end date {}: {}", end_date, e)),
        };
