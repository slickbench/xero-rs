...
async fn test_pay_calendar_api() -> miette::Result<()> {
    ...
    let result = match run_test(&mut client).await {
        Ok(_) => {
        ...
        .await
    {
        ...
        Err(e) => {
            return Err(miette::miette!(
                "Failed to retrieve created pay calendar: {:?}",
                e
            ));
        }
    };
    info!(
        "Retrieved the newly created pay calendar: {}",
        retrieved_pay_calendar.name
    );

    // Verify the retrieved pay calendar matches the created one
    assert_eq!(
        retrieved_pay_calendar.pay_calendar_id,
        created_pay_calendar.pay_calendar_id
    );
